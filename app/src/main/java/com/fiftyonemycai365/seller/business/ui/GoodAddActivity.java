...
import com.fiftyonemycai365.seller.business.adapter.NormsListAdapter;
 ...
 * 商品添加Activity
 */
public class GoodAddActivity extends BaseActivity implements BaseActivity.TitleListener, View.OnClickListener {
    ...
    private NormsListAdapter mNormsListAdapter;
    private ListView mNormsListView;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        initView();
        setPageTag(TagManage.GOOD_ADD_ACTIVITY);
    }
    ...
    private void initView() {
        ...
        mViewFinder.onClick(R.id.tv_add_norm, this);
        mViewFinder.onClick(R.id.ll_good_brief, this);

        mNormsListAdapter = new NormsListAdapter(this, new ArrayList<Norms>());
        mNormsListView = mViewFinder.find(R.id.lv_list);
        mNormsListView.setAdapter(mNormsListAdapter);
...
//                mNormsListAdapter.addNew();
    ...
    private void initNormsBlock() {
        EditText et = (EditText) findViewById(R.id.et_price);
        et.setFilters(new InputFilter[]{lengthfilter});
        if (mGoods != null) {
            et.setText(String.valueOf(mGoods.price));
        }

        et = (EditText) findViewById(R.id.et_stock);
        if (mGoods != null) {
            et.setText(String.valueOf(mGoods.stock));
        }

        reflashNormsBlock();
    }

    public void reflashNormsBlock() {
        if (mNormsListAdapter != null && mNormsListAdapter.getCount() > 0) {
            mViewFinder.find(R.id.ll_norms_container).setVisibility(View.GONE);
        } else {
            mViewFinder.find(R.id.ll_norms_container).setVisibility(View.VISIBLE);
        }
    }
    ...
    public void setTitle(TextView title, ImageButton left, View right) {
        title.setText(getString(R.string.msg_goods_add));
...
//        et = mViewFinder.find(R.id.et_desc);
//        et.setText(mGoods.brief);
        ...
        switch (view.getId()) {
            case R.id.rl_add_head:
                if (null == mPopupWinddow) {
                    mPopupWinddow = new ImageSwitcherPopupWindow(this, this, GoodAddActivity.class.getName());
                }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        switch (requestCode) {
            ...
            case CropHelper.REQUEST_GALLERY:
                ...
                try {
//                    Uri realUri = Uri.fromFile(new File(ImgUtils.getRealFilePath(this, photoUri)));
//                    updateHeadPhoto(ImgUtils.uriFormat(this, realUri).getPath());
    ...
    Handler hander = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            if (msg.what == 1) {
                ...
                ToastUtils.show(GoodAddActivity.this, R.string.msg_failed_update + obj);
            } else if (msg.what == 2) {
    ...
    public void reflashCurrNorms() {

        for (int i = 0; i < mNormsListAdapter.getCount(); i++) {
            View v = mNormsListView.getChildAt(i);
            EditText etx = (EditText) v.findViewById(R.id.et_price);
            if (!TextUtils.isEmpty(etx.getText().toString())) {
                mNormsListAdapter.getItem(i).price = Double.valueOf(etx.getText().toString()).doubleValue();
                try {
                    mNormsListAdapter.getItem(i).id = Integer.parseInt(etx.getTag().toString());
                } catch (Exception e) {
                    mNormsListAdapter.getItem(i).id = 0;
                }
            }

            etx = (EditText) v.findViewById(R.id.et_stock);
            if (!TextUtils.isEmpty(etx.getText().toString())) {
                mNormsListAdapter.getItem(i).stock = Integer.valueOf(etx.getText().toString()).intValue();
            }
            etx = (EditText) v.findViewById(R.id.et_model);
            if (!TextUtils.isEmpty(etx.getText().toString())) {
                mNormsListAdapter.getItem(i).name = etx.getText().toString();
            }
    ...
    private Goods createGoods() {
        ...
        goods.type = Constants.GOODS_TYPE.GOODS;
...
//        et = mViewFinder.find(R.id.et_desc);
//        goods.brief = et.getText().toString();
        goods.brief = mCurrGoodBiref;

        goods.norms = mNormsListAdapter.getmDatas();

        for (int i = 0; i < mNormsListAdapter.getCount(); i++) {
            View v = mNormsListView.getChildAt(i);
            EditText etx = (EditText) v.findViewById(R.id.et_price);
            if (!TextUtils.isEmpty(etx.getText().toString())) {
                goods.norms.get(i).price = Double.valueOf(etx.getText().toString()).doubleValue();
                try {
                    goods.norms.get(i).id = Integer.parseInt(etx.getTag().toString());
                } catch (Exception e) {
                    goods.norms.get(i).id = 0;
                }
            } else {
            ...
            etx = (EditText) v.findViewById(R.id.et_stock);
            if (!TextUtils.isEmpty(etx.getText().toString())) {
                goods.norms.get(i).stock = Integer.valueOf(etx.getText().toString()).intValue();
            } else {
                ToastUtils.show(this, getString(R.string.msg_import_repertory));
                return null;
            }
            etx = (EditText) v.findViewById(R.id.et_model);
            if (!TextUtils.isEmpty(etx.getText().toString())) {
                goods.norms.get(i).name = etx.getText().toString();
            } else {
                ToastUtils.show(this, getString(R.string.msg_import_type));
                return null;
            }
        ...
        if (ArraysUtils.isEmpty(goods.norms)) {
            ...
            et = mViewFinder.find(R.id.et_stock);
            if (!TextUtils.isEmpty(et.getText().toString())) {
                goods.stock = Integer.valueOf(et.getText().toString()).intValue();
            } else {
                ToastUtils.show(this, getString(R.string.msg_import_repertory));
                return null;
            }
        ...
        ApiUtils.post(this, URLConstants.GOODS_EDIT, mTmpGoods.getParams(),
                BaseResult.class, new Response.Listener<BaseResult>() {
                    ...
                    public void onResponse(BaseResult response) {
                        CustomDialogFragment.dismissDialog();
                        if (O2OUtils.checkResponse(GoodAddActivity.this, response)) {
                            if (mGoods != null) {
                                ToastUtils.show(GoodAddActivity.this, getString(R.string.msg_update_scceed));
                                EventBus.getDefault().post(new GoodOpEvent(null, false));
                            } else {
                                ToastUtils.show(GoodAddActivity.this, getString(R.string.msg_succ_add));
                    ...
                    public void onErrorResponse(VolleyError error) {
                        ToastUtils.show(GoodAddActivity.this, R.string.msg_error_update);
                        CustomDialogFragment.dismissDialog();
                    }
                });
    ...
    private void goodsOp(final int op) {
        ...
        ApiUtils.post(this, URLConstants.GOODS_OP, params,
                BaseResult.class, new Response.Listener<BaseResult>() {
                    ...
                    @Override
                    public void onErrorResponse(VolleyError error) {
                        ToastUtils.show(GoodAddActivity.this, R.string.msg_error);
                        CustomDialogFragment.dismissDialog();
                    }
                });
    }
