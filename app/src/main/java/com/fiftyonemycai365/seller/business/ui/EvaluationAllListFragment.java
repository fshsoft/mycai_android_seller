 ...
 */
public class EvaluationAllListFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setPageTag(TagManage.EVALUATION_ALL_LIST_FRAGMENT);
	}
		...
		mAdapter = new CommonAdapter<OrderRate>(mFragmentActivity, mDatas, R.layout.item_customer_reviews) {
			...
			public void convert(ViewHolder helper, OrderRate item,int position) {
				...
				if (null != item.reply){
					...
					((Button)helper.getView(R.id.customer_btn_reply)).setVisibility(View.GONE);
				}else if(null == item.reply){
					((Button)helper.getView(R.id.customer_btn_reply)).setVisibility(View.VISIBLE);
				}
				...
				helper.setText(R.id.customer_tv_name, item.goods.name);
	...
	private void loadData(final boolean isRefresh) {
		if (NetworkUtils.isNetworkAvaiable(mFragmentActivity)) {
			...
			data.put(ParamConstants.TYPE, String.valueOf(0));
