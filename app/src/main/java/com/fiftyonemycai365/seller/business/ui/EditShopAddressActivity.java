...
public class EditShopAddressActivity extends BaseActivity implements View.OnClickListener, TencentLocationListener, BaseActivity.TitleListener {
    private static final String TAG = EditShopAddressActivity.class.getName();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setPageTag(TagManage.EDIT_SHOP_ADDRESS_ACTIVITY);
    }
    private void searchAddressPoiByWebService(String regions, String keyword) {
        CustomDialogFragment.getInstance(getSupportFragmentManager(), TAG);
        ...
        ApiUtils.get(URLConstants.POI_SEARCH, paras, POIListResult.class, new Response.Listener<POIListResult>() {
            ...
            public void onResponse(POIListResult response) {
                ...
                if (response != null && response.status == 0 && response.count > 0 && !ArraysUtils.isEmpty(response.data)) {
                    ...
                    ToastUtils.show(EditShopAddressActivity.this, R.string.msg_unkown_loction);
        ...
        ts.search(param, new HttpResponseListener() {
            ...
            public void onSuccess(int i, Header[] headers, BaseObject results) {
                ...
                if (results != null) {
                    ...
                    } else ToastUtils.show(EditShopAddressActivity.this, getString(R.string.msg_unkown_loction));
                } else
                    ToastUtils.show(EditShopAddressActivity.this, getString(R.string.msg_unkown_loction));
            }
    ...
    private String geo2Address(double lat, double lng) {
        TencentSearch api = new TencentSearch(EditShopAddressActivity.this);
        ...
        api.geo2address(param, new HttpResponseListener() {
            ...
            public void onSuccess(int i, Header[] headers, BaseObject baseObject) {
                if (baseObject != null) {
                    ...
                    if (oj.result != null) {
                        ...
                        if (check.contains(getString(R.string.area_text))) {
                            String[] a = check.split(getString(R.string.area_text));
                            str = a[1];
                        }
                    }
                }
            ...
            @Override
            public void onFailure(int i, Header[] headers, String s, Throwable throwable) {
                ToastUtils.show(EditShopAddressActivity.this, getString(R.string.msg_error_invalid_location));
            }
        });
        return str;
    }
    ...
    public void setTitle(TextView title, ImageButton left, View right) {
        title.setText(getString(R.string.store_address_text));
    }
    ...
    public void onLocationChanged(TencentLocation tencentLocation, int error, String reason) {
        ...
        if (error == TencentLocation.ERROR_OK) {
            ...
            String s[] = r.split(getString(R.string.city));
            region = s[0];
            sb.append(getString(R.string.now_location_text)).append(tencentLocation.getProvince())
                    .append("，").append(tencentLocation.getCity());
            ToastUtils.show(EditShopAddressActivity.this, sb.toString());
            ...
            //定位失败
            ToastUtils.show(EditShopAddressActivity.this, reason);
            locationManager.removeUpdates(this);
        }
    }
    ...
    private void addMarker(final double lats, final double lngs) {
        ...
        OverlayItem marker = new OverlayItem(poi, getString(R.string.location), getString(R.string.label_text), markerImg);
        mapView.add(marker);
        ...
        api.geo2address(param, new HttpResponseListener() {
            ...
            public void onSuccess(int statusCode, Header[] headers, BaseObject object) {
                if (object != null) {
                    ...
                    if (oj.result != null) {
                        ...
                        ToastUtils.show(EditShopAddressActivity.this, mSelectedPoint.address);
                    } else ToastUtils.show(EditShopAddressActivity.this, getString(R.string.msg_error_address_error));
                } else ToastUtils.show(EditShopAddressActivity.this, getString(R.string.msg_error_address_error));
            }
            ...
            public void onFailure(int statusCode, Header[] headers,
                ...
                ToastUtils.show(EditShopAddressActivity.this, getString(R.string.msg_error_address_error));
            }
        });
    }
    ...
    public void onStatusUpdate(String name, int status, String desc) {
        ...
        if (status == STATUS_DENIED) {
			 ...
			 */
            Toast.makeText(this, getString(R.string.msg_error_lovation_jurisdiction), Toast.LENGTH_SHORT).show();
        }
    }
        ...
        switch (v.getId()) {
            case R.id.btn_search:
                if (TextUtils.isEmpty(mEdit.getText().toString().trim())) {
                    ToastUtils.show(this, getString(R.string.msg_error_search_no_null));
                    return;
                } else {
//                    searchAddressPoiByWebService(region, mEdit.getText().toString());
    ...
    private void saveAddress() {
        if (mSelectedPoint.x == 0 || mSelectedPoint.y == 0) {// || TextUtils.isEmpty(mSelectedPoint.address)) {
            ToastUtils.show(EditShopAddressActivity.this, getString(R.string.msg_error_nofound_lovation_hint));
            return;
        }
    ...
    private void editShop() {
        mShop.mapPointStr = mSelectedPoint.toString();
        mShop.address = mSelectedPoint.address;
        HashMap<String, Object> data = new HashMap<>();
        data.put("shopdatas", mShop);
        CustomDialogFragment.getInstance(getSupportFragmentManager(), EditShopAddressActivity.class.getName());
        ApiUtils.post(EditShopAddressActivity.this, URLConstants.SHOP_EDIT, data, ShopResult.class, new Response.Listener<ShopResult>() {
            @Override
            ...
            @Override
            public void onErrorResponse(VolleyError error) {
                ToastUtils.show(EditShopAddressActivity.this, R.string.msg_error_update);
                CustomDialogFragment.dismissDialog();
            }
        });
    }
